...
interface MyFeignClient {
     ...
     *
     * OpenFeign 이 비동기가 지원 되었다면.. @Async 스레드관점에서는 비동기 논블로킹이 가능했을 것이다.
     * -> 인터페이스 리턴타입으로 CompletableFuture 사용할 수 있다면..
     * -> -> 이렇게 되면 @Async 를 사용한 메서드에서 return xxx.getLatest2(); 로 처리할 수 있다..
     * 그러나, 여전히 OpenFeign 은 블로킹 방식이므로
     * @Async 스레드로 부터 위임 받은 OpenFeign 내부 자체 스레드는
     * 응답을 대기 하는 과정을 거쳐야한다. (블로킹)
     *
     * 그래서, OpenFeign 은 자체적으로 비동기를 지원할 이유가 없는 것이다.
     * 지원해도 .. 외부 서버 API 요청/응답이 블로킹 방식이므로.. 의미가 없다.
     * -> 어떤 스레드가 블로킹 할거냐? 그냥 계속 블로킹 책임을 미루는 것밖에 안된다.
     * 하지만, 외부 client 의 요청을 처리해주는 톰켓 스레드 관점에서는..
     * -> 해당 요청의 응답이 의미가 없다면.. 비동기로 한번 끊어주고 톰켓 스레드관점에서는 논블로킹을 해주는 것이좋다. (새로운 요청을 처리해줘야하므로..)
     * -> -> 물론.. 외부 server 로 요청 보내는 스레드는 블로킹 될 것이지만 말이다..
     *
     * 따라서, Application 전체 스레드 관점에서 논블로킹을 달성하기 위해서는
     * 논블로킹 I/O 를 지원하는 webflux 를 사용해야한다.
     */
    @GetMapping("/latest")
